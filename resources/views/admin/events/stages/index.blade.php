...
<div class="page-header">
    <div>
        ...
        <p class="page-subtitle">Manage event stages used for matches and Pick’em recommendations.</p>
    </div>
        ...
        <tbody>
            @forelse($event->stages as $stage)
                <tr>
                    ...
                    <td class="text-slate-300">{{ $stage->format ? str_replace('_', ' ', ucfirst($stage->format)) : '—' }}</td>
                    <td class="text-slate-300">{{ $stage->has_pickem ? 'Yes' : 'No' }}</td>
                    <td class="text-slate-300">{{ $stage->sort_order }}</td>
                    ...
                    <td class="text-right">
                        <div class="flex justify-end gap-2">
                            ...
                            <form method="POST" action="{{ route('admin.events.stages.destroy', [$event, $stage]) }}" onsubmit="return confirm('Delete this stage?');">
                                @csrf
                                @method('DELETE')
            ...
            @empty
                <tr>
                    <td colspan="6" class="empty-row">No stages yet.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
